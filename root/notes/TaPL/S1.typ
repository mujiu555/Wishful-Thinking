...
```
the constructors `TmTrue` to `TmIsZero` name the different sorts of nodes in the abstract syntax tree of type `term`;
Those type following `of` specifies the number of subtrees that will be attached to that type of node.

`info` denotes where the node originated. Which is generated by the parser.
