...
It is a functional, type-safe, type computed, lazy evaluated, programming language.

With lazy evaluation, it is possible to define infinite data structures.

Function is also a type.
It is possible to define a function using pattern match.
```haskell
not :: Bool -> Bool
not True = False
not False = True
...
You may define new types with `data` in Haskell, but since Haskell have no way to assign value to variable, it is not really a class in other OOP languages.
The way Haskell define classes is using type-classes.
